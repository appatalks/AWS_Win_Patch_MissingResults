...
instanceId="$2"
region="$1"
cmdId=$(aws ssm send-command --region "$region" --instance-ids "$instanceId" --document-name "AWS-RunShellScript" --query "Command.CommandId" --output text --parameters commands="'${@:3}'")
